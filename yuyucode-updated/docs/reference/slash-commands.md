...
All slash commands are handled in `src/hooks/useSlashCommands/` — a modular directory with per-domain handler files under `handlers/` and shared helpers under `helpers/`. The full list of ~68 commands is defined in `src/constants.js` under `SLASH_COMMANDS`.
...
| `/color <value>` | Set session bar colour: `red`, `green`, `blue`, or `off`. |
| `/ptt` | Toggle push-to-talk voice input mode. |
...
| `/apikeys` | Change Cerebras/Groq API keys at runtime — no APK rebuild required. Keys are AES-256-GCM encrypted before being stored. |
## Developer
