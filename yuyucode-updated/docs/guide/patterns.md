...
| All slash commands in `useSlashCommands/` | Modular directory — one handler file per domain |
...
Add a handler in `useSlashCommands/handlers/<domain>.js`, register in `handlers/index.js`, add entry to `SLASH_COMMANDS` in `src/constants.js`.
