...
features:
  ...
  - icon: 🗣️
    title: Wake Word
    details: Say "Hey Yuyu" (or "Oi Yuyu") and the agent activates — hands-free. Continuous Web Speech API detection with auto-restart on error. Toggle in /config.

  - icon: 🔐
    title: Encrypted Key Storage
    details: API keys encrypted at rest with AES-256-GCM + PBKDF2 (300K iterations). Keys expire after 24h and are never written in plaintext. Change them anytime with /apikeys — no APK rebuild needed.
...
The agent loop, visual diff review, background agents, multi-tab editor, perceptual brightness compensation, SQLite persistence, AES-GCM encrypted key storage, wake word detection — all of it runs on a Snapdragon 680 at 1235 tests passing, SonarCloud Security A · Blocker 0 · High 0, ~75% coverage.
