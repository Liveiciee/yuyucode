...
async function handleWatchMessage(rawData, folder, fileSnapshotsRef, setMessages, sendNotification) {
  const { event, filename } = JSON.parse(rawData);
  if (event === 'watching' || !filename) return;
  const safeFilename = String(filename).split('\u0000').join('').replace(/\.\.\//g, '');
  const absPath = folder + (safeFilename.startsWith('/') ? safeFilename : '/' + safeFilename);
  const prev = fileSnapshotsRef.current[absPath];
  const r = await callServer({ type: 'read', path: absPath });
  if (!r.ok) {
    setMessages(m => [...m, { role: 'assistant', content: '👁 **File berubah:** `' + filename + '`', actions: [] }]);
  ...
  if (!prev) {
    setMessages(m => [...m, { role: 'assistant', content: '👁 **File berubah:** `' + filename + '` _(snapshot awal disimpan)_', actions: [] }]);
    return;
  }
  const { added, removed } = computeFileDiff(prev, curr);
  ...
  setMessages(m => [...m, { role: 'assistant', content: `👁 **File berubah:** \`${filename}\` — ${summary}${diffText}`, actions: [] }]);
...
export default function App() {
  ...
  const { handleSlashCommand } = useSlashCommands({
    ...
    setShowDepGraph: ui.setShowDepGraph,
    setShowApiKeys, setDepGraph: ui.setDepGraph, setFontSize: ui.setFontSize,
