...
export function handleBatch({ parts, folder, abortRef, callAI, setLoading, setMessages }) {
  ...
  return withLoading(setLoading, async () => {
    ...
    const files = (listR.data || []).filter(f =>
      !f.isDir && (
        f.name.endsWith('.jsx') ||
        f.name.endsWith('.js') ||
        f.name.endsWith('.ts') ||
    ...
    for (const f of files) {
      ...
      const writes = Array.isArray(result) ? result : [];
      allWrites.push(...writes);
      simpleResponse(setMessages, '  ' + (writes.length > 0 ? '✅' : '⏭') + ' ' + f.name + (writes.length > 0 ? ' — ' + writes.length + ' perubahan' : ''));
    }
