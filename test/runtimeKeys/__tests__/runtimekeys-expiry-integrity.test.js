...
const createMockCrypto = () => {
  const localMockHashValue = 'mock-hash-1234567890';
  ...
  return {
    subtle: {
      ...
      digest: vi.fn().mockImplementation(async (_, data) => {
        ...
        if (input.includes('csk-valid-long-key')) return new TextEncoder().encode(localMockHashValue).buffer;
        if (input.includes('gsk-valid-long-key')) return new TextEncoder().encode(localMockHashValue).buffer;
        ...
        if (input.includes('csk-long-valid-key')) return new TextEncoder().encode(localMockHashValue).buffer;
        if (input.includes('gsk-long-valid-key')) return new TextEncoder().encode(localMockHashValue).buffer;
        return new TextEncoder().encode(localMockHashValue).buffer;
      }),
    },
