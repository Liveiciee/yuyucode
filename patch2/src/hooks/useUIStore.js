...
export function useUIStore() {
  ...
  function loadUIPrefs({ theme: t, fontSize: fs, sidebarWidth: sw, customTheme: ct, onboarded, vim: vm, minimap: mm, ghostText: gt, lint: lt, tslsp, blame, multiCursor: multiCursor_, stickyScroll: stickyScroll_, collab }) {
    if (t && THEME_KEYS.includes(t)) setThemeKeyRaw(t);
    else if (t) setThemeKeyRaw(DEFAULT_THEME);
    // Restore custom theme yang disimpan ThemeBuilder
    if (ct) { try { const parsed = JSON.parse(ct); if (parsed && typeof parsed === 'object') THEMES_MAP['custom'] = parsed; } catch (_e) { } }
