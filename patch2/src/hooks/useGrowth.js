...
export function useGrowth() {
  ...
  function addXP(event) {
    ...
    setXpRaw(prev => {
      ...
      Preferences.set({ key: 'yc_xp', value: String(newXp) });
      // Cek badge baru — pakai newXp, badges dari closure (badges jarang berubah, aman)
      const earned = BADGES.filter(b => !badges.includes(b.id) && b.req(newXp, streak));
      if (earned.length) {
        const newIds = [...badges, ...earned.map(b => b.id)];
        setBadgesRaw(newIds);
        Preferences.set({ key: 'yc_badges', value: JSON.stringify(newIds) });
        setNewBadge(earned[earned.length - 1]);
