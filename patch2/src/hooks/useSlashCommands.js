...
Tulis ke SKILL.md menggunakan write_file. Format singkat, padat, max 50 baris.`);
      ...
      const modelA = MODELS.find(m => m.provider === 'cerebras')?.id ?? MODELS[0].id;
      const modelB = MODELS.find(m => m.provider === 'groq')?.id ?? MODELS[2].id;
  ...
  // eslint-disable-next-line -- deps intentionally omit volatile fn refs (sendMsg, callAI, etc)
