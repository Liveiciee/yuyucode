...
export function useProjectStore() {
  ...
  const [hooks, setHooksRaw]           = useState({ preWrite: [], postWrite: [], postPush: [], preToolCall: [], postToolCall: [], onError: [] });
  ...
  function loadProjectPrefs({
    ...
    if (hk) { try { const saved = JSON.parse(hk); setHooksRaw(h => ({ ...h, ...saved })); } catch (_e) { } }
